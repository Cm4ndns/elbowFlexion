...
//upper arm and lower left arm (at near about 90deg is expected)
// function elbowPerp(obj14, obj16) {
//   const vector1 = [(obj14.x - obj14.x) , (obj14.y - obj14.y - 0.2)];
...
function elbowRange(obj14, obj16) {
  const vector1 = [(obj14.x - obj14.x) , (obj14.y - obj14.y - 0.2)];
  const vector2 = [(obj14.x - obj16.x) , (obj14.y - obj16.y)];
  const dot = vector1[0]*vector2[0] + vector1[1]*vector2[1];
  const mod_a = Math.sqrt(vector1[0]*vector1[0] + vector1[1]*vector1[1]);
...
//if 80deg ke upar hai tab, proceed (expected is to start with 90Deg)
    // let angle = elbowPerp( results.poseLandmarks[14], results.poseLandmarks[16]);
    // console.log(angle);
    const rangeAchieved = elbowRange(results.poseLandmarks[14], results.poseLandmarks[16]);
