...
//upper arm and lower right arm (at near about 90deg is expected)
// function elbowPerp(obj13, obj15) {
//   const vector1 = [(obj13.x - obj13.x) , (obj13.y - obj13.y - 0.2)];
...
//   const angle = 180 - Math.round((Math.acos(dot/(mod_a*mod_b))*180)/3.14);
//   return angle;
// }

function elbowRange(obj13, obj15) {
  const vector1 = [(obj13.x - obj13.x) , (obj13.y - obj13.y - 0.2)];
  const vector2 = [(obj13.x - obj15.x) , (obj13.y - obj15.y)];
  const dot = vector1[0]*vector2[0] + vector1[1]*vector2[1];
  const mod_a = Math.sqrt(vector1[0]*vector1[0] + vector1[1]*vector1[1]);
  const mod_b = Math.sqrt(vector2[0]*vector2[0] + vector2[1]*vector2[1]);
  let angle = 180 - ((Math.acos(dot/(mod_a*mod_b))*180)/3.14).toFixed(2);
...
//if 80deg ke upar hai tab, proceed (expected is to start with 90Deg)
    // let angle = elbowPerp( results.poseLandmarks[13], results.poseLandmarks[15]);
    // console.log(angle);
    const rangeAchieved = elbowRange(results.poseLandmarks[13], results.poseLandmarks[15]);
